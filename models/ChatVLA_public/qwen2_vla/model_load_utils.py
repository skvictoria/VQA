...
def find_all_linear_names(model, rank0_print, lora_module=None):
    ...
    multimodal_keywords = ['multi_modal_projector', 'policy_head', 'lm_head',
                           'xattn', 'input_action_proj', 'reasoning_action_proj',
                           'reasoning_film', 'merger', 'expert','vl_embed_action_proj']
    if 'vit' not in lora_module:
        # multimodal_keywords.append("vision_tower")
        multimodal_keywords.append("visual")
    if 'llm' not in lora_module:
        multimodal_keywords.append("layers")
    rank0_print("##" * 20)
...
def load_model(config=None, qwen2_vla_config=None, rank0_print=print, tokenizer=None):
    ...
    if training_args.load_pretrain:
        ...
        assert config[
                   'model_args'].model_pretrain != "", "load pretrain weights need set the model_pretrain in DataArguments!!!!"
        ...
        if config['training_args'].with_flash_attention:
        ...
        print(model)
    elif config['training_args'].with_flash_attention:
        model = AutoModelForCausalLM.from_pretrained(
    ...
    ################ for moe: maybe freeze expert ##################

    if config['model_args'].using_moe:
        for name, module in model.named_modules():
            if any(mm_keyword in name for mm_keyword in ['expert']):
                module.requires_grad_(True)
                if config['training_args'].freeze_vl_expert and 'static_expert.experts.0' in name:
                    module.requires_grad_(False)
                    rank0_print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Vl Expert freezed <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
...
# Borrowed from 1.utils.get_peft_model_state_dict
...
def load_model_for_eval(model_path, model_base, device_map="cuda",
    ...
    if 'qwen2' in model_path.lower() or 'qwen' in model_path.lower():
        ...
        if 'lora' in model_path.lower() and model_base is not None:
            if policy_config['pretrain_path'] is not None:
                if not os.path.exists(os.path.join(policy_config['pretrain_path'], 'pretrain_merge_weights')):
                    ...
                    model, tokenizer = load_merge_lora_weights(model_path=policy_config['pretrain_path'],
                                                               model_base=model_base, kwargs=kwargs)
                ...
                model, tokenizer = load_merge_lora_weights(model_path=model_path,
                                                           model_base=os.path.join(policy_config['pretrain_path'],
                                                                                   'pretrain_merge_weights'),
                                                           kwargs=kwargs)
            else:
        ...
        if policy_config['save_model']:
            print(
                f"#####################################Saving merged weights of model in {kwargs['torch_dtype']}"
                f".#####################################")
            save_path = model_path if not policy_config["enable_lora"] else os.path.join(model_path, 'merge_weights')
            skip_params = [
                "input_action_proj",
                "policy_head",
            ...
            torch.save(head_param, os.path.join(save_path, 'head_params.bin'))
            if "merge_weights" in save_path:
                os.makedirs(save_path, exist_ok=True)
